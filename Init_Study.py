...
def main():
	...
	while IDsCreated < number_of_study_IDs:
		...
		newID = study_modules.create_rnd_studyID( args.format,
			args.prefix, 
			args.suffix
			)
	...
	for ID in StudyIDs:
		...
		row += 1
	#----------------------> Log the creation <-----------------------
	study.log( f'New Study Created. Title: {args.title}, PI: {args.PI}',
	...
	print(f'\nProcess complete. Created {args.n} studyIDs of format ',end='')
