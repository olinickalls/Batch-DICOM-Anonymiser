...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default='uudddd',
        help='StudyID Format (U)pper, (L)ower, either case (C)har, (D)igit'
        )
    parser.add_argument(
        '-prefix',
    ...
    sample_format = studymodules.create_rnd_studyID(args.format, args.prefix)
    ...
    # sanity check - max possible vs requested StudyIDs
    max_no_IDs = studymodules.number_possible_IDs(args.format)
    ...
    if number_of_study_IDs > max_no_IDs:
        print(f'\n\tImpossible to create {number_of_study_IDs}'
              + ' Study IDs with current format.')
    ...
    # There is a faster (fewer collisions) way to do this with a list of all
    # possible IDs then randomly choose and remove from the list.
    # Dict may be faster than list.
    ...
    while IDsCreated < number_of_study_IDs:
        ...
        newID = studymodules.create_rnd_studyID(
            args.format,
            args.prefix,
            args.suffix
            )
        # prove the new studyID is not already present
        if newID not in StudyIDs:
            ...
            if IDsCreated >= Next_Landmark:
                print(f'progress: '
                      + f'{int((IDsCreated / number_of_study_IDs) * 100)}%')
    ...
    row = 2  # Starting row in datasheet
    for ID in StudyIDs:
        study.datasheet[study.XLSDATA_DEIDUID + str(row)] = ID
        row += 1
    # ----------------------> Log the creation <-----------------------
    study.log(
        f'New Study Created. Title: {args.title}, PI: {args.PI}',
        study.LOGLEVEL_NORMAL
        )
    study.log(
        f'Number of studyIDs:{args.n} Prefix: \"{args.prefix}'
        + '\" Format: \"{args.format}\"' + '{args.suffix}',
        study.LOGLEVEL_NORMAL
        )
    # --------------------->  Save XLSX file  <-------------------------
    ...
    except:  # noqa Catch ANY cause of failure to save
        print(f'Fatal Error: Failed to save \"{ xls_Filename }\"')
        raise
    else:
        print(f'Successfully saved \"{ xls_Filename }\"')
    print(f'\nComplete. Created {args.n} studyIDs of format ', end='')
    print(f'\"{args.prefix}\"+\"{args.format}\", saved as \"{xls_Filename}\"')
